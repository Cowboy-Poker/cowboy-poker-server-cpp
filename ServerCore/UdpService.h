...
public:
    static constexpr int64 SESSION_TIMEOUT_MS = 10000; // 10초 무응답 시 제거
    ...
    void BroadcastExcept(uint64 excludeSessionId, SendBufferRef sendBuffer);

    /* 메인 루프에서 주기적으로 호출 — 타임아웃 클라이언트 정리
       반환값: 이번 Tick에서 제거된 sessionId 목록 */
    Vector<uint64> Tick();
